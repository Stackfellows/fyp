...
const Reports = () => {
  ...
  const fetchReports = async (activeFilter) => {
    setLoading(true);
    try {
      const data = await getAdminReports(activeFilter);
      setReportData(data);
    } catch (err) {
        ...
        {loading ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={trendData}>
              ...
              />
              <Area type="monotone" dataKey="tickets"  name="Total Tickets" stroke="#003366" strokeWidth={2.5} fill="url(#gTickets)" />
              <Area type="monotone" dataKey="resolved" name="Resolved"       stroke="#d4af37" strokeWidth={2.5} fill="url(#gResolved)" />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </div>
